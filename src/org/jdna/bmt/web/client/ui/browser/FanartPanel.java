...
public class FanartPanel extends Composite {
	...
	private static FanartPanelUiBinder uiBinder = GWT
			.create(FanartPanelUiBinder.class);
	@UiField
	Image mainImage;
	@UiField
	...
	public void loadFanart(final MediaArtifactType type) {
		...
		controller.loadFanart(mediaFile, type,
				new AsyncCallback<ArrayList<GWTMediaArt>>() {
	...
	public void deleteImage(ClickEvent evt) {
		if (curImage != null) {
			if (Window
					.confirm("Press OK to delete this image.  This cannot be undone.")) {
				controller.getServices().deleteFanart(curImage.getMediaArt(),
						new AsyncCallback<Boolean>() {
							public void onFailure(Throwable caught) {
								Application.fireErrorEvent(
										"Unable to delete image", caught);
							}
							...
							public void onSuccess(Boolean result) {
								if (Boolean.TRUE.equals(result)) {
									...
									Application
											.fireErrorEvent("Unable to delete image");
								}
							}
						});
			}
...
//		}
	...
	public void makeDefaultImage(ClickEvent evt) {
		if (curImage != null) {
			controller.getServices().makeDefaultFanart(mediaFile, type,
					curImage.getMediaArt(), new AsyncCallback<Void>() {
						public void onFailure(Throwable caught) {
							Application.fireErrorEvent(
									"Unable to make this the default iamge",
									caught);
						}
						...
						public void onSuccess(Void result) {
							...
							//Application
							//		.fireNotification("This is now the default");
						}
					});
		}
	}
